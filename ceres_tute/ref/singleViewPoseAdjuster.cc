/*
 * Pose adjuster using keypoint likelihoods from a single image
...
int main(int argc, char** argv){
	...
	if(argc < 2){
		inputFileName = "ceres_input_chair_singleViewPoseAdjuster.txt";
		outputFileName = "ceres_output_chair_singleViewPoseAdjuster.txt";
	}
	else if(argc < 3){
...
*/
	...
	double *trans = myProblem.getTransIni();
	// Get initial Rotation
	double *rotMat = myProblem.getRotIni();
...
*/
	...
	for(int i = 0; i < numObs; ++i){
		...
		ceres::CostFunction *pnpError = new ceres::AutoDiffCostFunction<PnPError, 2, 3, 3>( 
			new PnPError(X_bar_initial+3*i, observations+2*i, curEigVec, K, observationWeights[i], lambdas, numEigV));
		// Add a residual block to the problem
		// ceres::HuberLoss(0.8) worked for most cases
		problem.AddResidualBlock(pnpError, NULL, rotAngleAxis, trans);
	...
	problem.AddResidualBlock(translationRegularizer, NULL, trans);
...
*/
	...
	// problem.SetParameterLowerBound(trans, 0, -2);
	// problem.SetParameterUpperBound(trans, 0, 2);
	// problem.SetParameterLowerBound(trans, 1, -2);
	// problem.SetParameterUpperBound(trans, 1, 2);
